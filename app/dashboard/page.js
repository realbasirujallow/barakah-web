    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid md:grid-cols-4 gap-6 mb-8">
          <StatCard title="Total Assets" value={`$${stats?.totalAssets?.toLocaleString() || 0}`} color="emerald" />
          <StatCard title="Monthly Income" value={`$${stats?.monthlyIncome?.toLocaleString() || 0}`} color="blue" />
          <StatCard title="Monthly Expenses" value={`$${stats?.monthlyExpenses?.toLocaleString() || 0}`} color="red" />
          <StatCard title="Net Income" value={`$${stats?.netIncome?.toLocaleString() || 0}`} color="green" />
        </div>
      ...
      {result && result.found && (
        <div className={`p-6 rounded-lg ${result.isHalal ? 'bg-green-50' : 'bg-red-50'}`}>
          <div className="flex items-start justify-between mb-4">
            ...
            <span className={`px-4 py-2 rounded-full font-semibold ${result.isHalal ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'}`}>

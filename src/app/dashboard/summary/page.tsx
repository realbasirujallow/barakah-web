...
function generateText(summary: PeriodSummary, months: MonthlyPoint[], period: string): string {
  ...
  if (months.length >= 2) {
    ...
    text += `Month-over-Month: Expenses ${chg >= 0 ? 'up' : 'down'} ${Math.abs(parseFloat(chgPct))}% vs prior month\n\n`;
  }
...
export default function SummaryPage() {
  ...
  const handleCopy = () => {
    if (!summary) return;
    const text = generateText(summary, monthly, period);
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm mb-5">
        ...
        <pre className="bg-gray-50 rounded-xl p-4 text-xs text-gray-700 whitespace-pre-wrap font-mono leading-relaxed">
          {generateText(summary, monthly, period)}
        </pre>
      </div>
    </div>
  );

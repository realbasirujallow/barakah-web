...
export default function ReferralPromptModal({ onDismiss }: Props) {
  // 2026-06-06 (QA LOC-7): modal previously rendered the title, CTAs, footnote,
  // close-aria, and the WhatsApp share body in English regardless of the
  // user's locale. The lead paragraph (REFERRAL_MODAL_LEAD) is sourced from
  // lib/referralCopy.ts and intentionally stays in English here — the reward
  // contract values it bakes in (months free, $X off) are validated server-
  // side against canonical USD copy. Founder to follow up on a localized
  // reward-contract pass if/when the campaign expands.
  ...
  const handleWhatsApp = () => {
    const message = encodeURIComponent(
      tFmt('referralModalWhatsAppBodyFmt', [shareUrl])
    );
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 pt-6 text-center animate-in fade-in zoom-in duration-300 relative max-h-[90vh] overflow-y-auto">
        ...
        <button
          ...
          aria-label={t('referralModalCloseAria')}
        ...
        {code && (
          <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-6">
            <p className="text-xs text-gray-500 mb-1">{t('referralModalYourCode')}</p>
        ...
        <div className="space-y-3">
          ...
          >
            {copied ? t('referralModalCopiedCta') : t('referralModalCopyLinkCta')}
          </button>
          ...
          >
            {t('referralModalMaybeLater')}
          </button>
        </div>

        <p className="text-xs text-gray-400 mt-4">
          {t('referralModalFootnote')}
        </p>
      </div>
    </div>
  );
